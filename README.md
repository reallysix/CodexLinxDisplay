...
- 可切换到“自定义图片”模式，选图后自动裁切并推送，同时暂停 Codex 用量读取。
...
2. 在“显示内容”中选择模式：
   ...
   - `自定义图片`：点击“选择图片…”，App 会将图片居中裁切为屏幕尺寸，保留顶部状态栏后推送，此时 Codex 自动刷新暂停。
...
Codex 用量在本机读取并渲染。App 不上传 Codex 凭据；它只会把生成后的 JPEG 或用户选中的图片发送到配置的图像 API，并访问 GitHub Release 更新源。选中的图片会在 App 的 Application Support 目录保存一份本地副本，用于下次启动继续显示。
